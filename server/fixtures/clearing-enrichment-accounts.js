if (Groups.find({name: 'IS200 Enrichment'}).count() > 0) {
    var users = [
    ...
    _.each(users, function (user) {
        var id = Meteor.users.findOne({'profile.name': user.name})._id;
        console.log('Removed %d user with name %s', Meteor.users.remove({'profile.name': user.name}), user.name);
        console.log('\tRemoved %d attempts belonging to %s', Attempts.remove({userId: id}), id);
    });

    console.log('Removed %d groups with name IS200 Enrichment', Groups.remove({name: 'IS200 Enrichment'}));
}